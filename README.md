...
---> 1.1) теперь работает, но ui не парсит протобаф нормально, хуй знает, полный рандом тут, надо просто пробовать
(короче тебе уже можно писать код, считай, кафка работает, на ui пофиг)

2) AI слой пока мокается одним стандартныфм сообщением (и то оно не доходит до кафки потому что пункт 1)
...
7) придумать схему бд чтоб нестыдно было, полей дофига, возможно, стоит жсоны хранить, тогда можно не посгрес а nosql бдшку какую-то
